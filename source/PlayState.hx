...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
        ...
        hits = perfects + sicks + goods + bads + shits + wtfs;
		...
		if (ClientPrefs.settings.get("scoreDisplay") == 'AMZ') {
			if (ratingFC != "") {
				hud.scoreTxt.text = 'Score: ${FlxStringUtil.formatMoney(songScore, false)} / Health: ${hud.healthBar.roundedPercent}% / NPS (Max): $notesPerSecond ($maxNps) / Accuracy: ${Highscore.floorDecimal(ratingPercent * 100, 2)}%';
			} else {
				hud.scoreTxt.text = 'Score: ${FlxStringUtil.formatMoney(songScore, false)} / Health: ${hud.healthBar.roundedPercent}% / NPS (Max): $notesPerSecond ($maxNps) / Accuracy: ${Highscore.floorDecimal(ratingPercent * 100, 2)}%';
			}
			var balls = npsArray.length - 1;
			...
			if(!boyfriend.stunned && generatedMusic && !endingSong)
			{
				...
				function noteCheck(note:Note) {
					if (note.canBeHit && !note.tooLate && !note.wasGoodHit)
					{
						...
						if (ratingIntensity == 'Harsh') canMiss = false;
					}
				}
	...
	public function recalculateRating() {
		...
		{
			case 'AMZ': hud.scoreTxt.text = 'Score: ${FlxStringUtil.formatMoney(songScore, false)} / Health: ${hud.healthBar.roundedPercent}% / NPS (Max): $notesPerSecond ($maxNps) / Accuracy: ${Highscore.floorDecimal(ratingPercent * 100, 2)}%';
			case 'Psych':
