...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var versionShit2:FlxText = new FlxText(6, FlxG.height - #if !html5 65 #else 44 #end, 0, 'Modified Denpa Engine v${Main.denpaEngineVersion.version}', 12);
		...
		add(versionShit2);
		var versionShit:FlxText = new FlxText(6, FlxG.height - #if !html5 45 #else 24 #end, 0, 'Friday Night Funkin${"'"} v${Application.current.meta.get('version')}', 12);
		versionShit.scrollFactor.set();
		versionShit.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		versionShit.active = false;
		add(versionShit);
		#if !html5
		var prompt:FlxText = new FlxText(6, FlxG.height - 25, 0, "R - Clear Save Data", 12);
