...
class HUD extends FlxSpriteGroup {
    ...
    public function new()
    {
		...
		if (ClientPrefs.settings.get("ratingsDisplay")) {
			...
			ratingsTxt.visible = !PlayState.instance.cpuControlled;
			add(ratingsTxt);
            updateRatings();
		}
		...
		scoreTxtBg.visible = !ClientPrefs.settings.get("hideHud") && !PlayState.instance.cpuControlled;
		...
		scoreTxt.visible = !ClientPrefs.settings.get("hideHud") && !PlayState.instance.cpuControlled;
    ...
    public function updateRatings() {
		...
		final rats = ["hits", "highestCombo", "combo", "perfects", "sicks", "goods", "bads", "shits", "wtfs", "misses"];
		final ratVars = [PlayState.instance.hits, PlayState.instance.highestCombo, PlayState.instance.combo, PlayState.instance.perfects, PlayState.instance.sicks, PlayState.instance.goods, PlayState.instance.bads, PlayState.instance.shits, PlayState.instance.wtfs, PlayState.instance.songMisses];
		for (i in 0...rats.length) {
			curRatings.set(rats[i], ratVars[i]);
		}
        ratingsTxt.text = 'Hits: ${curRatings.get("hits")}\nMax Combo: ${curRatings.get("highestCombo")}\nCombo: ${curRatings.get("combo")}\nPerfects: ${curRatings.get("perfects")}\nSicks: ${curRatings.get("sicks")}\nGoods: ${curRatings.get("goods")}\nBads: ${curRatings.get("bads")}\nShits: ${curRatings.get("shits")}\nWTFs: ${curRatings.get("wtfs")}\nMisses: ${curRatings.get("misses")}';
    }
